...
async def test_prepare_workspace(client_test: AsyncClient):
    ...
    response = await client_test.post("/v1/workspaces",
    ...
    response = await client_test.post(f"/v1/workspaces/{workspace.id}/members", json={"accounts": members},
...
async def test_create_group(client_test: AsyncClient):
    ...
    response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups",
                                     headers={"Authorization": f"Bearer {active_user.token}"})
    assert response.status_code == status.HTTP_200_OK
    response = response.json()
    ...
    # Create the groups
    for group in groups:
        response = await client_test.post(f"/v1/workspaces/{workspace.id}/groups",
...
async def test_get_group_wrong_id(client_test: AsyncClient):
    ...
    response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{random_id}",
                                     headers={"Authorization": f"Bearer {active_user.token}"})
...
async def test_get_group_info(client_test: AsyncClient):
    ...
    response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}",
                                     headers={"Authorization": f"Bearer {active_user.token}"})
    assert response.status_code == status.HTTP_200_OK
    response = response.json()
    assert response["name"] == group.name
    ...
    response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}/members",
                                     headers={"Authorization": f"Bearer {active_user.token}"})
    assert response.status_code == status.HTTP_200_OK
    response = response.json()
...
async def test_update_group_info(client_test: AsyncClient):
    ...
    response = await client_test.patch(f"/v1/workspaces/{workspace.id}/groups/{group.id}",
                                       json={"name": group.name, "description": group.description},
                                       headers={"Authorization": f"Bearer {active_user.token}"})
    assert response.status_code == status.HTTP_200_OK
    response = response.json()
...
async def test_add_members_to_group(client_test: AsyncClient):
    ...
    response = await client_test.post(f"/v1/workspaces/{workspace.id}/groups/{group.id}/members", json={"accounts": members},
                                      headers={"Authorization": f"Bearer {active_user.token}"})
    assert response.status_code == status.HTTP_200_OK
    response = response.json()
    for i in response["members"]:
...
async def test_get_user_policy(client_test: AsyncClient):
    ...
    response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}/policies",
...
async def test_permissions(client_test: AsyncClient):
    ...
    headers = {"Authorization": f"Bearer {active_user.token}"}
    res = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}", headers=headers)
    ...
    res = await client_test.patch(f"/v1/workspaces/{workspace.id}/groups/{group.id}",
                                  json={"name": "New name", "description": "New description"},
                                  headers=headers)
    assert res.status_code == status.HTTP_403_FORBIDDEN

    # Try to delete group
    res = await client_test.delete(f"/v1/workspaces/{workspace.id}/groups/{group.id}", headers=headers)
    assert res.status_code == status.HTTP_403_FORBIDDEN

    # Try to get group members
    res = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}/members", headers=headers)
    assert res.status_code == status.HTTP_403_FORBIDDEN

    # Try to add members to group
    res = await client_test.post(f"/v1/workspaces/{workspace.id}/groups/{group.id}/members",
    ...
    for member in members:
        if member["account_id"] == accounts[2].id:
            member_id = member["id"]

    res = await client_test.delete(f"/v1/workspaces/{workspace.id}/groups/{group.id}/members/{member_id}",
                                   headers=headers)
    assert res.status_code == status.HTTP_403_FORBIDDEN

    # Try to get group permissions
    res = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}/policies", headers=headers)
    assert res.status_code == status.HTTP_200_OK
    policy = res.json()["policies"][0]

    # Try to set group permissions
    res = await client_test.put(f"/v1/workspaces/{workspace.id}/groups/{group.id}/policies/{policy['id']}",
...
async def test_set_permissions(client_test: AsyncClient):
    ...
    response = await client_test.put(f"/v1/workspaces/{workspace.id}/groups/{group.id}/policies/{policy['id']}",
...
async def test_delete_group(client_test: AsyncClient):
    ...
    for group in groups:
        ...
        response = await client_test.get(f"/v1/workspaces/{workspace.id}/groups/{group.id}", headers=headers)
        assert response.status_code == status.HTTP_200_OK
        response = response.json()
        assert response["name"] == group.name
        ...
        response = await client_test.delete(f"/v1/workspaces/{workspace.id}/groups/{group.id}", headers=headers)
    ...
    response = await client_test.delete(f"/v1/workspaces/{workspace.id}", headers=headers)
    assert response.status_code == status.HTTP_204_NO_CONTENT
