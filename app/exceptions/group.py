...
class UserAlreadyExists(HTTPException):
    def __init__(self, user: User, group: Group):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User {user.email} already exists in group {group.name}") 
...
class UserNotInGroup(HTTPException):
    def __init__(self, user: User, group: Group):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User {user.email} not in group {group.name}") 
...
class UserNotAuthorized(HTTPException):
    def __init__(self, user: User, group: Group, action: str = "perform this action in"):
        super().__init__(status_code=status.HTTP_403_FORBIDDEN, detail=f"User {user.email} is not authorized to {action} group {group.name}") 
...
class GroupNotFound(HTTPException):
    def __init__(self, group_id: PydanticObjectId):
        super().__init__(status_code=status.HTTP_404_NOT_FOUND, detail=f"Group with id {group_id} not found") 
